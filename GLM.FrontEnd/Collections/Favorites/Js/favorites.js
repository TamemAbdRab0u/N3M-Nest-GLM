...
async function loadGames(page = 1, query = '', genre = '', platform = '', ordering = '', release = '', status = '') {
    ...
    try {
        ...
        if (genre) {
            gamesData = gamesData.filter(g => g.genres.some(name => {
        ...
        if (platform) {
            const platformMap = { '1': 'pc', '2': 'playstation', '3': 'xbox', '4': 'ios', '5': 'mac', '6': 'linux', '7': 'nintendo', '8': 'android' };
            const platSlug = platformMap[platform];
            if (platSlug) gamesData = gamesData.filter(g => g.platforms.some(p => p.toLowerCase().includes(platSlug)));
        }
        if (release) {
            const [start, end] = release.split(',');
            const startDate = new Date(start), endDate = new Date(end);
            gamesData = gamesData.filter(g => { const d = new Date(g.releaseDate); return d >= startDate && d <= endDate; });
        }
        if (ordering) {
...
function renderGames(games) {
    ...
    if (!container) return;
    if (games.length === 0) {
        container.innerHTML = `<div class="col-span-full flex flex-col items-center justify-center py-20 text-slate-500"><span class="material-symbols-outlined text-6xl mb-4 opacity-20">favorite</span><p class="text-xl font-medium">No favorite games found</p></div>`;
        return;
    }
    container.innerHTML = '';
...
// Minimal versions of other initializers for brevity
async function initializeCategories() { }
async function initializePlatforms() { }
async function initializeReleaseYears() { }
function setRatingOrdering(o, l) { loadGames(1, currentQuery, currentGenre, currentPlatform, o); }
function setStatusFilter(s, l) { loadGames(1, currentQuery, currentGenre, currentPlatform, currentOrdering, currentRelease, s); }
function clearAllFilters() { loadGames(1); }
function showGameDetails(game) { window.location.href = `../../../GameDetails/Html/game-details.html?id=${game.externalId || game.id}`; }
...
function renderInventoryIndicatorsHTML(game) {
    return `
        <div class="flex justify-end gap-1 min-w-[65px] h-8 items-center mt-2 relative">
            ...
            ${game.isFavorite ? '<div class="h-[30px] w-[40px] rounded-full border border-red-500/30 flex items-center justify-center backdrop-blur-sm" title="Favorited"><span class="material-symbols-outlined text-[15px] text-red-500 fill-icon scale-110">favorite</span></div>' : ''}
            ${game.isInLibrary ? '<div class="h-[30px] w-[40px] rounded-full border border-green-500/30 flex items-center justify-center backdrop-blur-sm" title="In Library"><span class="material-symbols-outlined text-[15px] text-green-500 fill-icon scale-110">inventory_2</span></div>' : ''}
        </div>
    `;
}
...
function updateFavoriteUI(gameId, isFavorite) {
    ...
    // Since this is the favorites page, remove it from view if unfavorited
    if (!isFavorite) {
